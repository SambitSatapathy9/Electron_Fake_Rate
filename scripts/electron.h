...
// Wed Feb 22 21:43:32 2023 by ROOT version 6.12/07
...
public :
   ...
   //   electron(const char* file1, const char* file2);
   electron(TString input);
   virtual ~electron();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TChain *tree);
   virtual void     Loop(string outputfile, int maxEvents);
   //virtual void     Loop(int maxEvents);
   ...
   float EaChargedHadrons(float);
   float EaNeutralHadrons(float);
   float EaPhotons       (float);
   float EaChargedWorst  (float);
...
#ifdef electron_cxx
electron::electron(TString input)
{
  TChain *chain = new TChain("phoJetNtuplizer/eventTree"); //created a chain of files that contains information in the eventTree
  TSystemDirectory dataDir("data",input); //Access the directory where the Data is located
  TList* fileList = dataDir.GetListOfFiles(); //Access all the files located in the directory of the data
  TIter next(fileList); //Iterate over the files in the directory
  TSystemFile* fileName; //Represents a file or a directory in the file system
  
  //initialise file number
  int filenum = 0;
  int maxfiles = -1;
  
  while((fileName = (TSystemFile*)next()) && filenum > maxfiles)
    {
      TString dataset = "Data_"; //It will check the file having the string "Data_"
      TString inputpath = (input+fileName -> GetName()); //
  ...
  std::cout<<"Initialising chain"<<std::endl;
  Init(chain); //Initialising the chain
...
void electron::Init(TChain *tree)
{
